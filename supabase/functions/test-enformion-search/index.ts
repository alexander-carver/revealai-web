...
serve(async (req) => {
  console.log("[TEST-enformion-search] ⚠️ TEST FUNCTION - MOBILE VERSION");
  ...
  console.log(`[TEST-enformion-search] Base URL: ${ENFORMION_ENDPOINT}`);
  console.log(`[TEST-enformion-search] Operation: ${operation}`);
  console.log(`[TEST-enformion-search] Incoming: ${JSON.stringify(payload)}`);

  try {
    switch (operation as Operation) {
      case "person_candidates":
        return await proxy("PersonSearch", payload, ["Person", "DevAPIPersonSearch"]);
      case "person_profile":
        return await proxy("PersonSearch", payload, ["Person"], PERSON_SEARCH_INCLUDES_STARTER, true);
      case "reverse_phone":
      ...
      default:
        return new Response(`Unsupported operation: ${operation}`, { status: 400 });
    }
  } catch (e) {
    console.error("[TEST-enformion-search] Error:", e);
...
async function proxy(path: string, base: Record<string, unknown>, searchType: string | string[], includes?: string[], requireIdForIncludes = false) {
  ...
  if (wantIncludes) {
    ...
    if (hasId) {
      ...
      console.log(`[TEST-enformion-search] Using TahoeIds-only payload`);
    } else {
      payload.Includes = includes;
    }
  } else {
    if (payload.TahoeIds) {
      console.log("[TEST-enformion-search] Stripping TahoeIds for candidate search");
      delete payload.TahoeIds;
    }
  }

  console.log(`[TEST-enformion-search] Final payload:\n${JSON.stringify(payload, null, 2)}`);
  ...
  for (const type of searchTypes) {
    ...
    for (const url of candidatePaths) {
      console.log(`[TEST-enformion-search] Calling ${url}`);
      console.log(`[TEST-enformion-search] Headers: galaxy-search-type=${type}, client-type=${ENFORMION_CLIENT_TYPE}`);
      ...
      console.log(`[TEST-enformion-search] Response status: ${resp.status}`);
      console.log(`[TEST-enformion-search] Response (first 400): ${text.substring(0, 400)}`);
