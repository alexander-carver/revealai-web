          ...
          onClick={() => {
            ...
            if (printWindow) {
              printWindow.document.write(`
                <!DOCTYPE html>
                <html><head><title>RevealAI Report${personName ? ` - ${personName}` : ""}</title>
                <style>
                ...
                <h1>RevealAI Report${personName ? `: ${personName}` : ""}</h1>
                ...
                ${report.sources.length > 0 ? `<h2>Sources</h2><ul>${report.sources.map(s => `<li><a href="${s.url}">${s.label}</a></li>`).join("")}</ul>` : ""}
                <div class="footer">Report generated by RevealAI &middot; revealai-peoplesearch.com</div>
                </body></html>
              `);
            ...
            if (navigator.share) {
              try {
                await navigator.share({
                  title: `RevealAI Report${personName ? ` - ${personName}` : ""}`,
